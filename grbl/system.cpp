...
{
#if MotherBoard==3
		SET_INPUT(RESET_INPUT_PIN);
		SET_INPUT(FEED_HOLD_INPUT_PIN);
		SET_INPUT(CYCLE_INPUT_PIN);
		PULLUP(RESET_INPUT_PIN,HIGH);
		PULLUP(FEED_HOLD_INPUT_PIN,HIGH);
		PULLUP(CYCLE_INPUT_PIN,HIGH);
		SET_INT_INPUT(RESET_INPUT_PIN);
		SET_INT_INPUT(FEED_HOLD_INPUT_PIN);
		SET_INT_INPUT(CYCLE_INPUT_PIN);
#else
...
ISR(PINOUT_INT_vect) 
{
  ...
  ///////////////////////////////////////########################**********************
#if MotherBoard==3
	  if(READ(RESET_INPUT_PIN)==false){
		  mc_reset();
	  }
	  if(READ(FEED_HOLD_INPUT_PIN)==false){
		  bit_true(sys.execute, EXEC_FEED_HOLD); 
	  }
	  if(READ(CYCLE_INPUT_PIN)==false){
		  bit_true(sys.execute, EXEC_CYCLE_START);
	  }
#else
	// Enter only if any pinout pin is actively low.
	if ((PINOUT_PIN & PINOUT_MASK) ^ PINOUT_MASK) { 
	  if (bit_isfalse(PINOUT_PIN,MASK(PIN_RESET))) {
		mc_reset();
	  } else if (bit_isfalse(PINOUT_PIN,MASK(PIN_FEED_HOLD))) {
		bit_true(sys.execute, EXEC_FEED_HOLD); 
	  } else if (bit_isfalse(PINOUT_PIN,MASK(PIN_CYCLE_START))) {
		bit_true(sys.execute, EXEC_CYCLE_START);
	  } 
	}
#endif
