  ...
  // Enable stepper drivers.
   //////////////////######################********************************************************************
	  ...
	  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { 
		  WRITE(X_ENABLE_PIN,X_DRIVER_EN_LOGIC);
#if FEATURE_SECOND_X
		  WRITE(X2_ENABLE_PIN,X2_DRIVER_EN_LOGIC);
#endif
		  WRITE(Y_ENABLE_PIN,Y_DRIVER_EN_LOGIC);
#if FEATURE_SECOND_Y
		  WRITE(Y2_ENABLE_PIN,Y2_DRIVER_EN_LOGIC);
#endif
		  WRITE(Z_ENABLE_PIN,Z_DRIVER_EN_LOGIC);
#if FEATURE_SECOND_Z
		  WRITE(Z2_ENABLE_PIN,Z2_DRIVER_EN_LOGIC);
#endif
	  }
	  else { 
		  WRITE(X_ENABLE_PIN,(~X_DRIVER_EN_LOGIC & 1));
#if FEATURE_SECOND_X
		  WRITE(X2_ENABLE_PIN,(~X2_DRIVER_EN_LOGIC & 1));
#endif
		  WRITE(Y_ENABLE_PIN,(~Y_DRIVER_EN_LOGIC & 1));
#if FEATURE_SECOND_Y
		  WRITE(Y2_ENABLE_PIN,(~Y2_DRIVER_EN_LOGIC & 1));
#endif
		  WRITE(Z_ENABLE_PIN,(~Z_DRIVER_EN_LOGIC & 1));
#if FEATURE_SECOND_Z
		  WRITE(Z2_ENABLE_PIN,(~Z2_DRIVER_EN_LOGIC & 1));
#endif
	  }
 #else    ///***MB!=3
...
// SPINDLE_ENABLE_PORT ^= 1<<SPINDLE_ENABLE_BIT; // Debug: Used to time ISR
...
#if MotherBoard==3 /////****MB==3
	// Set the direction pins a couple of nanoseconds before we step the steppers
	  WRITE(X_DIR_PIN,(U8_PIN(st.dir_outbits,X_MASK)^X_DRIVER_DIR_INVERTING));	////*****Write into X  dir pin	
#if FEATURE_SECOND_X
	  WRITE(X2_DIR_PIN,(U8_PIN(st.dir_outbits,X_MASK)^X_DRIVER_SECOND_DIR_INVERTING));	 ////*****Write into X	dir pin  
#endif
	  WRITE(Y_DIR_PIN,(U8_PIN(st.dir_outbits,Y_MASK)^Y_DRIVER_DIR_INVERTING)); 
#if FEATURE_SECOND_Y
	  WRITE(Y2_DIR_PIN,(U8_PIN(st.dir_outbits,Y_MASK)^Y_DRIVER_SECOND_DIR_INVERTING)); 
#endif
	  WRITE(Z_DIR_PIN,(U8_PIN(st.dir_outbits,Z_MASK)^Z_DRIVER_DIR_INVERTING));	///***U8_PIN(Va,Ma) trans byte to bit
#if FEATURE_SECOND_Z
	  WRITE(Z2_DIR_PIN,(U8_PIN(st.dir_outbits,Z_MASK)^Z_DRIVER_SECOND_DIR_INVERTING));	 ///***U8_PIN(Va,Ma) trans byte to bit
#endif
  ...
  /////////////*****************read pull up state
	  uint8_t bits=0;
	  ///////****do not need the seconde stepper info, they are same step
	  if(GET_PORT(X_STEP_PIN)) {bits|=MASK(X_MASK);}
	  if(GET_PORT(Y_STEP_PIN)) {bits|=MASK(Y_MASK);}
	  if(GET_PORT(Z_STEP_PIN)) {bits|=MASK(Z_MASK);}
	  ///////****do not need the seconde stepper info, they are same step
	  
	  st.step_bits = (bits & ~STEP_MASK) | st.step_outbits;    // Store out_bits to prevent overwriting.
  //	st.step_bits =st.step_outbits; // Store out_bits to prevent overwriting.  
  #else  // Normal operation
  ///////////////////////////////############################**********************
	  WRITE(X_STEP_PIN,U8_PIN(st.step_outbits,X_MASK));   ////*****Write into X step pin
#if FEATURE_SECOND_X
	  WRITE(X2_STEP_PIN,U8_PIN(st.step_outbits,X_MASK));   ////*****Write into X step pin
#endif
	  WRITE(Y_STEP_PIN,U8_PIN(st.step_outbits,Y_MASK));
#if FEATURE_SECOND_Y
		  WRITE(Y2_STEP_PIN,U8_PIN(st.step_outbits,Y_MASK));   ////*****Write into Y step pin
#endif
	  WRITE(Z_STEP_PIN,U8_PIN(st.step_outbits,Z_MASK)); 
#if FEATURE_SECOND_Z
	   WRITE(Z2_STEP_PIN,U8_PIN(st.step_outbits,Z_MASK));	////*****Write into Z step pin
#endif
   ///////////////////////////////############################**********************
  #endif  ////****end STEP_PULSE_DELAY
  ...
  if (st.counter_x > st.exec_block->step_event_count) {
	  ////////////////////////#####################********************
#if MotherBoard==3   ///****MB==3
		  st.step_outbits |= MASK(X_MASK);	 /////***step mask is same to dir mask and limit mask
		  st.counter_x -= st.exec_block->step_event_count;
		  if (st.exec_block->direction_bits & MASK(X_MASK)) { sys.position[X_AXIS]--; }
  ...
  if (st.counter_y > st.exec_block->step_event_count) {
	  ////////////////////////#####################********************
#if MotherBoard==3   ///****MB==3
		  st.step_outbits |= MASK(Y_MASK); /////***step mask is same to dir mask and limit mask
		  st.counter_y -= st.exec_block->step_event_count;
		  if (st.exec_block->direction_bits & MASK(Y_MASK)) { sys.position[Y_AXIS]--; }
  ...
  if (st.counter_z > st.exec_block->step_event_count) {
	  ////////////////////////#####################********************
#if MotherBoard==3   ///****MB==3  
		  st.step_outbits |= MASK(Z_MASK);	/////***step mask is same to dir mask and limit mask
		  st.counter_z -= st.exec_block->step_event_count;
		  if (st.exec_block->direction_bits & MASK(Z_MASK)) { sys.position[Z_AXIS]--; }
...
#if MotherBoard==3
   /////////////*****************read pull up state
	  WRITE(X_STEP_PIN, U8_PIN(settings.step_invert_mask,X_MASK));	 ////*****Write into X step pin
#if FEATURE_SECOND_X
	  WRITE(X2_STEP_PIN, U8_PIN(settings.step_invert_mask,X_MASK));   ////*****Write into X step pin
#endif
	  WRITE(Y_STEP_PIN, U8_PIN(settings.step_invert_mask,Y_MASK));
#if FEATURE_SECOND_Y
	  WRITE(Y2_STEP_PIN, U8_PIN(settings.step_invert_mask,Y_MASK));   ////*****Write into X step pin
#endif
	  WRITE(Z_STEP_PIN, U8_PIN(settings.step_invert_mask,Z_MASK)); 
#if FEATURE_SECOND_Z
	  WRITE(Z2_STEP_PIN, U8_PIN(settings.step_invert_mask,Z_MASK));   ////*****Write into X step pin
#endif
#else  ///***MB!=3
...
#if MotherBoard==3
	 /////////////*****************read pull up state
		WRITE(X_STEP_PIN, U8_PIN(st.step_bits,X_MASK));   ////*****Write into X step pin
#if FEATURE_SECOND_X
		WRITE(X2_STEP_PIN,U8_PIN(st.step_bits,X_MASK));  ////*****Write into X step pin
#endif
		WRITE(Y_STEP_PIN, U8_PIN(st.step_bits,Y_MASK));
#if FEATURE_SECOND_Y
		WRITE(Y2_STEP_PIN,U8_PIN(st.step_bits,Y_MASK));  ////*****Write into Y step pin
#endif
		WRITE(Z_STEP_PIN, U8_PIN(st.step_bits,Z_MASK)); 
#if FEATURE_SECOND_Z
		WRITE(Z2_STEP_PIN,U8_PIN(st.step_bits,Z_MASK));   ////*****Write into Z step pin
#endif
...
void st_reset()
{
  ...
  #if MotherBoard==3   ///###MB==3
#if X_STEP_PIN>-1    /////***set step output mode
	PULLUP(X_STEP_PIN, U8_PIN(step_port_invert_mask,X_MASK));	////*****Write into X step pin
#if FEATURE_SECOND_X
	PULLUP(X2_STEP_PIN, U8_PIN(step_port_invert_mask,X_MASK));	////*****Write into X step pin
#endif
#endif
#if Y_STEP_PIN>-1
	PULLUP(Y_STEP_PIN, U8_PIN(step_port_invert_mask,Y_MASK));  /////***step port initialize
#if FEATURE_SECOND_Y
	PULLUP(Y2_STEP_PIN, U8_PIN(step_port_invert_mask,Y_MASK));  /////***step port initialize
#endif
#endif
#if Z_STEP_PIN>-1
	PULLUP(Z_STEP_PIN, U8_PIN(step_port_invert_mask,Z_MASK));
#if FEATURE_SECOND_Z
	PULLUP(Z2_STEP_PIN, U8_PIN(step_port_invert_mask,Z_MASK));
#endif
#endif
	
#if X_DIR_PIN>-1      /////***set dir output mode
	PULLUP(X_DIR_PIN, (U8_PIN(dir_port_invert_mask,X_MASK)^X_DRIVER_DIR_INVERTING));
#if FEATURE_SECOND_X
	PULLUP(X2_DIR_PIN, (U8_PIN(dir_port_invert_mask,X_MASK)^X_DRIVER_SECOND_DIR_INVERTING));	
#endif
#endif
#if Y_DIR_PIN>-1
    PULLUP(Y_DIR_PIN, (U8_PIN(dir_port_invert_mask,Y_MASK)^Y_DRIVER_DIR_INVERTING));
#if FEATURE_SECOND_Y
    PULLUP(Y2_DIR_PIN, (U8_PIN(dir_port_invert_mask,Y_MASK)^Y_DRIVER_SECOND_DIR_INVERTING));
#endif
#endif
#if Z_DIR_PIN>-1
	PULLUP(Z_DIR_PIN, (U8_PIN(dir_port_invert_mask,Z_MASK)^Z_DRIVER_DIR_INVERTING));
#if FEATURE_SECOND_Z 
	PULLUP(Z2_DIR_PIN, (U8_PIN(dir_port_invert_mask,Z_MASK)^Z_DRIVER_SECOND_DIR_INVERTING));	
#endif
#endif
#if N_AXIS>3
/////#################for later 4 	AXIS
...
void stepper_init()
{
...
#endif
#endif
#if X_ENABLE_PIN>-1     /////***set  enable output mode
	  SET_OUTPUT(X_ENABLE_PIN);    ////***three enable pin
#if FEATURE_SECOND_X
  SET_OUTPUT(X2_ENABLE_PIN);   ////***three enable pin
#endif
#endif
#if Y_ENABLE_PIN>-1    
  ...
  /////#################for later 4   AXIS
	...
	STEP_DDR |= STEP_MASK;	 /////***set step output mode
	STEPPERS_DISABLE_DDR |= 1<<STEPPERS_DISABLE_BIT;	   /////***set	enable output mode
	DIRECTION_DDR |= DIRECTION_MASK;	   /////***set dir output mode
