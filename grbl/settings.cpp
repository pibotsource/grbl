...
void settings_restore_global_settings() {  
  ...
  /////////////////////////#################////////////////////////////////////
#if MotherBoard==3
/*  unsigned char dir_inverting_temp=0;
	if(X_DRIVER_DIR_INVERTING)
	 dir_inverting_temp |= MASK(X_AXIS);
	if(Y_DRIVER_DIR_INVERTING)
	 dir_inverting_temp |= MASK(Y_AXIS);
	if(Z_DRIVER_DIR_INVERTING)
	 dir_inverting_temp |= MASK(Z_AXIS);
	settings.dir_invert_mask = dir_inverting_temp;	 */
	//////////*****************need chang  but not dealing yet
...
uint8_t get_step_pin_mask(uint8_t axis_idx)       ///////**** home bug come from here
{
#if MotherBoard==3
	if ( axis_idx == X_AXIS ) { return((1<<X_MASK)); }
	if ( axis_idx == Y_AXIS ) { return((1<<Y_MASK)); }
	return((1<<Z_MASK));
#else
...
uint8_t get_direction_pin_mask(uint8_t axis_idx)    ///////**** home bug come from here
{
#if MotherBoard==3
		if ( axis_idx == X_AXIS ) { return((1<<X_MASK)); }
		if ( axis_idx == Y_AXIS ) { return((1<<Y_MASK)); }
...
uint8_t get_limit_pin_mask(uint8_t axis_idx)        ///////**** home bug come from here
{
#if MotherBoard==3 && !defined(limit_int_style)
		if ( axis_idx == X_AXIS ) { return((1<<X_MASK)); }
		if ( axis_idx == Y_AXIS ) { return((1<<Y_MASK)); }
		return((1<<Z_MASK));
#else
